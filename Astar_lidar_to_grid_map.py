...
def main():
    ...
    # Print coordinates for each square in the occupancy grid
    for y in range(occupancy_map.shape[0]):
        for x in range(occupancy_map.shape[1]):
            print(f"({x}, {y}): {occupancy_map[y, x]}")
    ...
    for goal in goals:
        ...
        if path:
            path = np.array(path)
            print(f"Path to {goal}: {path.tolist()}")
            ax.plot(path[:, 1], path[:, 0], 'r--', linewidth=2)
        else:
            print(f"No valid path to {goal}")
